...
// oldsz newsz for controling the va of user space in kernel page table
void ukvmcopy(pagetable_t pagetable, pagetable_t kernelpt, uint64 oldsz, uint64 newsz) {
  pte_t *pte_from, *pte_to;
  oldsz = PGROUNDUP(oldsz);
  for(uint64 i = oldsz; i < newsz; i += PGSIZE) {
    if((pte_from = walk(pagetable, i, 0)) == 0) {
      panic("ukvmcopy: src pte walk error");
    }
    if((pte_to = walk(kernelpt, i, 1)) == 0) {
      panic("ukvmcopy: kernelpt walk error");
    }
    uint64 pa = PTE2PA(*pte_from);
    uint64 flags = (PTE_FLAGS(*pte_from)) & (~PTE_U);
...
void vmprint_with_prefix(pagetable_t pagetable, char *prefix) {
  for(int i=0;i<512;i++) {
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0) {
      uint64 child = PTE2PA(pte);
      printf("%s%d: pte %p pa %p\n", prefix, i, pte, child);
      vmprint_with_prefix((pagetable_t) child, ".. .. ..");
    } else if(pte & PTE_V) {
